...
class StartNode : public rclcpp::Node
{
    public:
        ...
        : Node(node_name, rclcpp::NodeOptions().use_intra_process_comms(true)), count_(0), trace_callbacks_(trace_ptr), exe_time_(exe_time), period_(period), end_flag_(end_flag)
        {
    ...
    private:
        ...
        void timer_callback()
        {
            ...
            message.stamp.nanosec = ctime.tv_usec;
            ...
            if (end_flag_) {
                ...
                latency_time.tv_usec = (ftime.tv_usec - message.stamp.nanosec);                
...
class IntermediateNode : public rclcpp::Node
{
    public:
        ...
        : Node(node_name, rclcpp::NodeOptions().use_intra_process_comms(true)), count_(0), trace_callbacks_(trace_ptr), exe_time_(exe_time), end_flag_(end_flag)
        {                        
    ...
    private:
        ...
        void callback(const std_msgs::msg::String::SharedPtr msg) {
            ...
            if (end_flag_) {
                ...
                latency_time.tv_usec = (ftime.tv_usec - msg->stamp.nanosec);
                trace_callbacks_->trace_write_count(name+"_latency",std::to_string(latency_time.tv_sec*1000000+latency_time.tv_usec), message.data);  
            }
...
int main(int argc, char * argv[])
{
    ...
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "PID: %d run in ROS2.", syscall(SYS_gettid));
