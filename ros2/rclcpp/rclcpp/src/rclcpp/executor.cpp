...
{
  {
    ...
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "number of ready timer is %d", wait_set_.size_of_timers);
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "number of ready subscriptions is %d", wait_set_.size_of_subscriptions);
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "number of ready clients is %d", wait_set_.size_of_clients);
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "number of ready services is %d", wait_set_.size_of_services);    
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "=================================");    
    */
#endif
...
#ifdef HJ_DEBUG
  //RCLCPP_INFO_STREAM(rclcpp::get_logger("rclcpp"), "wait_for_work stimeout is " << std::chrono::duration_cast<std::chrono::nanoseconds>(timeout).count());
#endif
...
Executor::get_next_ready_executable(AnyExecutable & any_executable)
{
  ...
  if (callback_priority_enabled) {
    ...
    if (highest_priority >= 0) return true;
  }
  else { // default ROS2 executor
